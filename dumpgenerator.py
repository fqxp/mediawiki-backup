...
def urlopen(config, request):
    if config['username'] is not None:
        password_mgr = urllib2.HTTPPasswordMgrWithDefaultRealm()
        password_mgr.add_password(None, request.get_full_url(), config['username'], config['password'])
...
def getParameters(params=[]):
    ...
    #console params
    try:
        opts, args = getopt.getopt(params, "", ["h", "help", "path=", "api=", "index=", "images", "logs", "xml", "curonly", "resume", "cookies=", "delay=", "namespaces=", "exnamespaces=", "force", "username=", "password=", ])
...
def checkIndexphp(config):
    """ Checking index.php availability """
    indexphp = config['index']
